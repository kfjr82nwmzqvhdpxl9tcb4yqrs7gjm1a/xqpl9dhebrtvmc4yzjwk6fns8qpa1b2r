...
(async () => {
  try {
    await pool.query(`
      ...
      ADD COLUMN IF NOT EXISTS welcome_message TEXT DEFAULT '👋 Welcome @user to @group!';
    `);
    console.log('✅ Columns ensured for welcome feature');
  } catch (err) {
    console.error('❌ Failed to ensure columns in group_settings:', err);
  }
})();
